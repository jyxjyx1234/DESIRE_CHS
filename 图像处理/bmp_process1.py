...
def replace_rect(src_path, replacement_path, output_path,text):
    ...
    replacement_img = replacement_img.crop((71, 0, 326, 64))

    # 将矩形部分替换为替换文件的内容
    src_img.paste(replacement_img, (71, 0, 326, 64))

    # 加入文字
    from PIL import ImageDraw, ImageFont
    draw = ImageDraw.Draw(src_img)
    font = ImageFont.truetype("C:\Windows\Fonts\msyhbd.ttc", 30)  # 使用合适的字体文件路径
    draw.text((77, 8), text , font=font, fill="white")
...
for file in filenames:
    ...
    replace_rect("sysgrp_bmp1\\"+file, "sysgrp_bmp_origin\\background.bmp", "sysgrp_bmpout\\"+file, text)
